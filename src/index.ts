...
try {
    ...
    if (!db.contentGoals?.postingCadence) console.warn("⚠️ 'contentGoals.postingCadence' missing in DB, using defaults.");
    if (!db.contentStrategies?.dailyPrompts) throw new Error("Missing required 'contentStrategies.dailyPrompts'");
...
try {
    ...
    console.error(`❌ Error loading state file, initializing empty state.`, error);
    state = {};
}
...
function pickRandom<T>(arr: T[]): T {
    if (!arr || arr.length === 0) throw new Error("Cannot pick random from empty array.");
...
async function safeGenerateText(prompt: string, maxLength = AI_CHAR_LIMIT): Promise<string | null> {
    ...
    try {
        console.log(`🧠 Generating AI text (max ${maxLength} chars, aiming for natural style)...`);
        ...
        if (error.message?.includes('SAFETY')) {
            ...
            console.warn(`   Prompt start that may have caused block: ${prompt.substring(0,150)}...`);
        }
        return null;
    }
}
...
async function safePostTweet(text: string, options?: { reply?: { in_reply_to_tweet_id: string } }): Promise<TweetV2PostTweetResult | null> {
    ...
    try {
        ...
        if (error.code === 429) {
            console.error(`   RATE LIMIT HIT (429). Need to run less frequently or post less.`);
        } else if (error.code) {
...
async function postTweet(): Promise<boolean> {
    ...
    try {
        ...
        if (!prompts?.length) { console.warn("⚠️ No daily tweet prompts. Skipping."); return false; }
        ...
        if (!text) { console.error("❌ Failed to generate AI text for tweet."); return false; }
        const postResult = await safePostTweet(text);
        return !!postResult;
    } catch (error) { console.error('❌ Unexpected error during postTweet:', error); return false; }
}
...
async function generateThreadParts(pillar: ContentPillar, seedPrompt: string): Promise<string[] | null> {
    try {
        const structures = db.contentStrategies.threadStructures;
         if (!structures?.length) { console.warn("⚠️ No thread structures in DB."); return null; }
        ...
        for (const [index, section] of structure.format.entries()) {
            // Updated prompt for human-like style
            const partPrompt = `You are ${db.founderProfile.name}, voice: ${db.founderProfile.voice}. Writing part ${index + 1}/${structure.format.length} of a Twitter thread on '${pillar.name}' (Idea: '${seedPrompt}'). Focus this part on: '${section}'. Target audience: ${db.audience.target}.`;
            const partText = await safeGenerateText(partPrompt);
            if (!partText) {
                console.error(`❌ Failed to generate part ${index + 1}. Aborting thread generation.`);
                return null;
            }
    ...
    } catch (error) { console.error(`❌ Error generating thread parts:`, error); return null; }
}
...
async function postThread(): Promise<boolean> {
    ...
    try {
        ...
        if (!pillars?.length) { console.warn("⚠️ No content pillars. Skipping."); return false; }
        if (!prompts?.length) { console.warn("⚠️ No daily thread prompts. Skipping."); return false; }
        ...
        if (!generatedParts || generatedParts.length < MIN_THREAD_PARTS) {
            console.error("❌ Thread generation failed or produced too few raw parts.");
            return false;
        }
        ...
        console.log(`🐦 Posting thread with ${finalPartsToPost.length} parts (randomly selected length)...`);

        // Post first part
        let firstTweetResult = await safePostTweet(finalPartsToPost[0]);
        if (!firstTweetResult) { console.error("❌ Failed to post first tweet. Aborting."); return false; }
        ...
        for (let i = 1; i < finalPartsToPost.length; i++) {
            ...
            console.log(`   ...waiting ${partDelay}ms before posting part ${i + 1}...`);
        ...
        return true;
    } catch (error) { console.error('❌ Unexpected error during postThread:', error); return false; }
}
...
async function runTask() {
    ...
    console.log(`🎲 Calculated probability to post this run: ${probability.toFixed(2)} (Remaining: ${remainingPosts}, Approx Runs Left: ${remainingRunsToday})`);
    ...
    if (canPostThread && canPostTweet) {
        ...
        console.log(`💡 Both types possible, randomly chose: ${postTypeAttempted}.`);
    ...
    if (postedSuccessfully && postTypeAttempted) {
        console.log(`✅ ${postTypeAttempted} posted. New counts: Threads ${counters.threads}, Tweets ${counters.tweets}`);
